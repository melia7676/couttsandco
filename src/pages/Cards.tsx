...
import { getCardsByUserId, formatCurrency, getAccountById } from "@/data/users";
...
export default function Cards() {
  ...
  const userCards = user ? getCardsByUserId(user.id, profile?.full_name || undefined) : [];

  const [cardStates, setCardStates] = useState<Record<string, boolean>>(
    userCards.reduce((acc, card) => ({ ...acc, [card.id]: card.isLocked }), {})
  );
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {userCards.map((card, index) => {
          ...
          const isLocked = cardStates[card.id] ?? card.isLocked;
