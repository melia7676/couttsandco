...
export default function Dashboard() {
  ...
  // Real user display info from Supabase
  const displayName = profile?.full_name || user.email?.split("@")[0] || "Valued Client";
  const memberSince = user.created_at
    ? new Date(user.created_at).getFullYear().toString()
    : "2024";

  // Mock financial data (keep existing data layer)
  // NOTE: If your mock data uses old dummy user IDs, you may need to update
  // @/data/users to map accounts/transactions to your new Supabase user.id
  ...
  // Compute total balance from accounts instead of old dummy user.totalBalance
  ...
  return (
    <div className="space-y-6 lg:space-y-8">
      {/* Welcome Header - Real user data from Supabase */}
      <motion.div
        ...
        <div className="relative z-10">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between lg:gap-6">
            <div className="min-w-0">
              <p className="text-primary-foreground/70 text-xs sm:text-sm font-medium mb-1 sm:mb-2">Total Account Balance</p>
              <BalanceDisplay amount={totalBalance} size="xl" className="text-primary-foreground" />
              <p className="text-primary-foreground/70 text-xs sm:text-sm mt-1 sm:mt-2 truncate">
