...
import { getInvestmentsByUserId, formatCurrency } from "@/data/users";
import { Card } from "@/components/ui/card";
import { Button } from "@/components/ui/button";

export default function Investments() {
  const { user } = useAuth();
  const userInvestments = user ? getInvestmentsByUserId(user.id) : [];

  const totalValue = userInvestments.reduce((sum, inv) => sum + inv.value, 0);
  const totalGainLoss = userInvestments.reduce((sum, inv) => sum + inv.gainLoss, 0);
    ...
    <div className="space-y-8">
      ...
      >
        ...
        <Card className="p-6">
          ...
          <p className={`text-2xl font-bold ${totalGainLossPercent >= 0 ? "text-success" : "text-destructive"}`}>
        ...
        {userInvestments.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="text-muted-foreground text-sm max-w-md mx-auto">
              Your investment portfolio is empty. Start trading to build your portfolio.
            </p>
          </div>
        ) : (
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    {investment.shares} shares • Avg cost {formatCurrency(investment.avgCost)}
                  </p>
                </div>
                ...
                <div className="text-right">
                  ...
                  <div className={`flex items-center gap-1 justify-end ${investment.gainLoss >= 0 ? "text-success" : "text-destructive"}`}>
