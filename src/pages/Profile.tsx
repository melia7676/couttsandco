...
export default function Profile() {
  ...
  // Derive real user info from Supabase
  ...
  const state = ""; // Not collected in signup, can be added later
  const zip = profile?.postcode || "";
  const country = profile?.country || "";

  // Year from Supabase account creation
  const memberSince = user?.created_at
    ? new Date(user.created_at).getFullYear().toString()
    : "2024";

  // New users are "Basic" members
  const tier = "Basic";
        ...
        <div className="p-6 border-b border-border/50 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground">{tier} Member since {memberSince}</p>
            </div>
          </div>
          <Button
